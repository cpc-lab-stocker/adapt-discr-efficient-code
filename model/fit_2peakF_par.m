% 2peakF: increased coding accuracy at adaptor & 90deg off, allow different total Fisher from control condition
...
load(['fit_ctrl_4522.5_sub' num2str(subj) '.mat'], 'theta_o', 'kappa_e', 'k_o', 'kappa_o', 'x');

%% adaptation
theta = [0, 90];
params0 = [2, 0.5/180, 80, 2, 100];
vlb = [-inf, -0, 0.01, 1e-4, 1]; 
vub = [inf, 1/180, 700, 700, 700];
...
negLog_adapt_prior( params0, x, k_o, theta_o, kappa_o, theta, kappa_e, adaptor_45(1), test_45, dtheta_45, nTot_exp_45, nRight_exp_45, adaptor_225(1), test_225, dtheta_225, nTot_exp_225, nRight_exp_225, res, epsilon )
profile on
[fit_params, negLog_exp] = fminsearchbnd(@(params)negLog_adapt_prior( params, x, k_o, theta_o, kappa_o, theta, kappa_e, adaptor_45(1), test_45, dtheta_45, nTot_exp_45, nRight_exp_45, adaptor_225(1), test_225, dtheta_225, nTot_exp_225, nRight_exp_225, res, epsilon ),params0,vlb,vub)
profile off
BIC_exp = 2*negLog_exp + length(vlb)*log(sum(nTot_exp_45,'all')+sum(nTot_exp_225,'all'));

k21 = fit_params(1);
pmin = fit_params(2);
kappa = fit_params(length(theta)+1:2*length(theta));
kappa_i1 = fit_params(2*length(theta)+1);
totFisher1 = sqrt(kappa_i1 * besseli(1, kappa_i1) / besseli(0, kappa_i1)) *2*pi;
...
[ ~, ~, ~, thresh_exp_pred_45 ] = ECAdapt_2AFC_par( [k_o k_o], theta_o, [kappa_o kappa_o], k, theta, kappa, kappa_i1, kappa_e, res, adaptor_45(2), adaptor_45(2)+x );
[ ~, ~, ~, thresh_exp_pred_225 ] = ECAdapt_2AFC_par( [k_o k_o], theta_o, [kappa_o kappa_o], k, theta, kappa, kappa_i1, kappa_e, res, adaptor_225(2), adaptor_225(2)+x );

save(['fit_' FIT_VER '_4522.5_sub' num2str(subj) '.mat'], 'theta', 'k21', 'pmin', 'k', 'kappa', 'kappa_i1', 'negLog_exp', 'BIC_exp', 'totFisher1', 'x', 'fit_kernel', 'thresh_exp_pred_45', 'thresh_exp_pred_225');



x2 = -90:res:90-res; % detheta grid

[ x1, probCmpCw ] = ECAdapt_2AFC_par( [k_o k_o], theta_o, [kappa_o kappa_o], k, theta, kappa, kappa_i1, kappa_e, res, adaptor_45(1), adaptor_45(1)+test_45 );
...
[ x1, probCmpCw ] = ECAdapt_2AFC_par( [k_o k_o], theta_o, [kappa_o kappa_o], k, theta, kappa, kappa_i1, kappa_e, res, adaptor_225(1), adaptor_225(1)+test_225 );
...
function negLog = negLog_adapt_prior( params, x, k_o, theta_o, kappa_o, theta, kappa_e, adaptor1, test1, dtheta1, nTot1, nRight1, adaptor2, test2, dtheta2, nTot2, nRight2, res, epsilon )
    k21 = params(1);
    pmin = params(2);
    kappa = params(length(theta)+1:2*length(theta));
    kappa_i = params(2*length(theta)+1);
...
function negLog = negLog_adapt_prior_one( params, x, k_o, theta_o, kappa_o, theta, kappa_e, adaptor, test, dtheta_pool, nTot, nRight, res, epsilon )
    k21 = params(1);
    pmin = params(2);
    kappa = params(length(theta)+1:2*length(theta));
    kappa_i = params(2*length(theta)+1);
