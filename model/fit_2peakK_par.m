% 2peakK: increased coding accuracy at adaptor & 90deg off, allow different kernels for 45 & 22.5 adaptors
% fit to 45 & 22.5 oblique conditions separatly, with parallel computing
clear all;
epsilon = 2e-2;
res = 0.5;
FIT_VER = '2peakK';
...
%% adaptation
theta = [0, 90];
params0 = [2, 0.5/180, 80, 2];
vlb = [-inf, -0, 0.01, 1e-4]; 
vub = [inf, 1/180, 700, 700];
...
% 45
negLog_adapt_prior_one( params0, x, k_o, theta_o, kappa_o, theta, kappa_i, kappa_e, adaptor_45(1), test_45, dtheta_45, nTot_exp_45, nRight_exp_45, res, epsilon )
profile on
[fit_params, negLog_exp_45] = fminsearchbnd(@(params)negLog_adapt_prior_one( params, x, k_o, theta_o, kappa_o, theta, kappa_i, kappa_e, adaptor_45(1), test_45, dtheta_45, nTot_exp_45, nRight_exp_45, res, epsilon ),params0,vlb,vub)
profile off
BIC_exp_45 = 2*negLog_exp_45 + length(vlb)*log(sum(nTot_exp_45,'all'));

k21_45 = fit_params(1);
pmin_45 = fit_params(2);
kappa_45 = fit_params(length(theta)+1:2*length(theta));

k_ = [1, k21_45];
p_ = sum_n_vmpdf_180( x, k_, theta, kappa_45 ) - (1-sum(k_))/180;
pmin_ = min(p_);
k_45 = k_ * (pmin_45-1/180) / (pmin_-(k21_45+1)/180);

fit_kernel_45 = sum_n_vmpdf_180( x, k_45, theta, kappa_45 );

save(['fit_' FIT_VER '_4522.5_sub' num2str(subj) '.mat'], 'x', 'theta', 'k21_45', 'pmin_45', 'k_45', 'kappa_45', 'negLog_exp_45', 'BIC_exp_45', 'fit_kernel_45');

% 22.5
negLog_adapt_prior_one( params0, x, k_o, theta_o, kappa_o, theta, kappa_i, kappa_e, adaptor_225(1), test_225, dtheta_225, nTot_exp_225, nRight_exp_225, res, epsilon )
profile on
[fit_params, negLog_exp_225] = fminsearchbnd(@(params)negLog_adapt_prior_one( params, x, k_o, theta_o, kappa_o, theta, kappa_i, kappa_e, adaptor_225(1), test_225, dtheta_225, nTot_exp_225, nRight_exp_225, res, epsilon ),params0,vlb,vub)
profile off
BIC_exp_225 = 2*negLog_exp_225 + length(vlb)*log(sum(nTot_exp_225,'all'));

k21_225 = fit_params(1);
pmin_225 = fit_params(2);
kappa_225 = fit_params(length(theta)+1:2*length(theta));

k_ = [1, k21_225];
p_ = sum_n_vmpdf_180( x, k_, theta, kappa_225 ) - (1-sum(k_))/180;
pmin_ = min(p_);
k_225 = k_ * (pmin_225-1/180) / (pmin_-(k21_225+1)/180);

fit_kernel_225 = sum_n_vmpdf_180( x, k_225, theta, kappa_225 );

save(['fit_' FIT_VER '_4522.5_sub' num2str(subj) '.mat'], 'k21_225', 'pmin_225', 'k_225', 'kappa_225', 'negLog_exp_225', 'BIC_exp_225', 'fit_kernel_225', '-append');

% together
negLog_exp = negLog_exp_45 + negLog_exp_225;
BIC_exp = BIC_exp_45 + BIC_exp_225;

[ ~, ~, ~, thresh_exp_pred_45 ] = ECAdapt_2AFC_par( [k_o k_o], theta_o, [kappa_o kappa_o], k_45, theta, kappa_45, kappa_i, kappa_e, res, adaptor_45(2), adaptor_45(2)+x );
[ ~, ~, ~, thresh_exp_pred_225 ] = ECAdapt_2AFC_par( [k_o k_o], theta_o, [kappa_o kappa_o], k_225, theta, kappa_225, kappa_i, kappa_e, res, adaptor_225(2), adaptor_225(2)+x );

x2 = -90:res:90-res; % detheta grid

[ x1, probCmpCw ] = ECAdapt_2AFC_par( [k_o k_o], theta_o, [kappa_o kappa_o], k_45, theta, kappa_45, kappa_i, kappa_e, res, adaptor_45(1), adaptor_45(1)+test_45 );
...
[ x1, probCmpCw ] = ECAdapt_2AFC_par( [k_o k_o], theta_o, [kappa_o kappa_o], k_225, theta, kappa_225, kappa_i, kappa_e, res, adaptor_225(1), adaptor_225(1)+test_225 );
...
save(['fit_' FIT_VER '_4522.5_sub' num2str(subj) '.mat'], 'negLog_exp', 'BIC_exp', 'thresh_exp_pred_45', 'thresh_exp_pred_225', 'x2', 'probCmpCw_exp_45', 'probCmpCw_exp_225', '-append');
